...
void std_constructors_capacity_test() {
	...
	std::list<Test> uselesss(s.begin(), s.end());
...
void iter_test()
{
	...
	std::list<int>::iterator cit(def.begin());
	while (cit != def.end())
		std::cout << *(cit++) << std::endl;

	std::list<int>::reverse_iterator rcit(def.rbegin());
		while (rcit != def.rend())
